...
retrieve_from_file (const char *file, bool html, int *count)
{
  ...
  *count = 0;                  /* Reset the URL count.  */
  ...
  gettimeofday(&limit_timer, NULL);
  limit_timer.tv_sec 	+= opt.wait_op_time / 1000000;
  ...
  for (cur_url = url_list; cur_url; cur_url = cur_url->next, ++*count)
    {
      ...
      diff_time = time_diff( limit_timer, current_timer );

	  if ( diff_time >= opt.wait_op_time ) {
  	      break;
	  }
